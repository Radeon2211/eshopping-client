...
export default function OrderDetails() {
  ...
  if (orderDetails === null) {
    content = (
      <Heading $variant="h4" $align="center" $lineHeight="level4" data-testid="OrderDetails-error">
      ...
      <div data-testid="OrderDetails-general-info-section">
        ...
        <FlexWrapper $direction="column" $spacing="level1">
          <div>
            ...
            <PlainText $size="level3">{_id}</PlainText>
          </div>
          <div>
            <PlainText $weight="700">Transaction date:&nbsp;</PlainText>
            <PlainText $size="level3">{formatOrderDate(createdAt)}</PlainText>
          </div>
          <div>
    ...
    const infoAboutSellerSection = (
      <div data-testid="OrderDetails-info-about-seller">
        ...
        {seller ? (
          <FlexWrapper $direction="column" $spacing="level1" data-test="OrderDetails-seller-info">
            ...
            <div data-testid="OrderDetails-seller-email">
              ...
              <PlainText $size="level3">{seller.email}</PlainText>
            </div>
            <div data-testid="OrderDetails-seller-phone">
              <PlainText $weight="700">Phone:&nbsp;</PlainText>
              <PlainText $size="level3">{seller.phone}</PlainText>
            </div>
          </FlexWrapper>
        ) : (
    ...
    const productsSection = (
      <>
        ...
        <PlainText $display="block" $mgTop="level3" $textAlign="right">
          <PlainText $color={theme.colors.light4} $spacing="1px" $mgRight="level1">
            TOTAL
          </PlainText>
          <PlainText $size="level5">{formatPrice(overallPrice)}</PlainText>
        </PlainText>
      </>
    );

...
describe('<CartItem />', () => {
  ...
  describe('Checks how everything render', () => {
    it('Should render correctly with full data', () => {
      const data = createCartItem('u1', 'user1', 5, 'p1', 299.98, 'productName', 6, 'data:image');
      ...
      expect(wrapper.find('.photo').prop('src')).toBe(`${baseURL}/products/p1/photo`);
      expect(wrapper.find('.name').text()).toBe('productName');
      expect(wrapper.find('.quantity-number').text()).toBe('of 6');
      expect(wrapper.find('.overall-price').text()).toBe('$1,499.9');
      expect(wrapper.find('.price-per-piece').first().text()).toBe('per piece $299.98');
    });

    it('Should NOT render price per piece and should render default photo', () => {
      const data = createCartItem();
      const wrapper = setUp(data);
      expect(wrapper.find('.photo').prop('src')).toBe(noPhoto);
